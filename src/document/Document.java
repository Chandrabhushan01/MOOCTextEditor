...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		{
		    char c=cArray[i];
		    if(count>0&&numS&&i==l-1&&Character.toLowerCase(c)=='e')
			{
				count--;
			}
			if(vow.indexOf(Character.toLowerCase(c))>=0&&numS)
			{
				numS=false;
				count++;
			}
			else if(vow.indexOf(Character.toLowerCase(c))<0)
			{
				numS=true;
			}
	...
	public double getFleschScore()
	{
		...
		//System.out.println(sentFound+" "+wordsFound+" "+syllFound);
		
		result=206.835-((1.015*(wordsFound/sentFound))+(84.6*(syllFound/wordsFound)));
