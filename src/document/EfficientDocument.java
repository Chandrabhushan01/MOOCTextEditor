...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for( String st:tokens)                //loop through each string of list tokens
		{
			if(isWord(st))                   //condition to check each string is word
			{
				numWords++;
				numSyllables+=countSyllables(st); //counting no of syllables in each word
			}
			else
			{
				numSentences++;    //counting no of sentence using no of sequence of punctuation
			}	
		}
		int n=tokens.size();      //size of tokens
		
		if(n!=0&&isWord((String)tokens.get(n-1))) //increase no of sentence if last token is a word
		{
			numSentences++;
		}
